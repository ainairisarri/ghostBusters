...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Definir cada connexión entre variables
    edges = [
        (GHOST0, OBS0),
        (PAC, OBS0),
        (PAC, OBS1),
    ...
    # Definir los dominios de las variables
    # Dominio de la variable Pacman
    position_domain = []
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            position_domain.append((x, y))

    # Dominio de distancia observada
    max_manhattan_distance = (X_RANGE - 1) + (Y_RANGE - 1)
    max_observed_distance = max_manhattan_distance + MAX_NOISE
    distance_domain = list(range(0, max_observed_distance + 1))
    ...
    variableDomainsDict[OBS0] = distance_domain
    variableDomainsDict[OBS1] = distance_domain
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        total_sum = self.total()
        if total_sum == 0:
            raise Exception("Cannot sample from an empty or zero-sum distribution.")
        rand_value = random.uniform(0, total_sum)
        cumulative_sum = 0.0
